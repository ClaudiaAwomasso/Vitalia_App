...
class FirebaseService {
  ...
  Future<CentreModel?> registerCentre({
    ...
    try {
      ...
      // 2️⃣ Création du modèle du centre
      ...
      await _firestore.collection('centres').doc(uid).set({
        ...centreData.toMap(),
        'statut': 'Actif',
  ...
  Future<CentreModel?> loginCentre({
    ...
    try {
      ...
      // 2️⃣ Récupération des données Firestore
...
class FirebaseService {
  ...
  /// Inscription d’un centre avec tous les champs
  Future<CentreModel?> registerCentre({
    required String email,
    required String password,
    required String nom,
    ...
    try {
      ...
      // Création du modèle du centre avec les valeurs du formulaire
      final centreData = CentreModel(
        id: uid,
        nom: nom,
        adresse: adresse,
      ...
      // Sauvegarde dans Firestore avec tous les champs
      await _firestore.collection('centres').doc(uid).set(centreData.toMap()..addAll({
        'statut': 'Actif',
        'createdAt': FieldValue.serverTimestamp(),
      }));

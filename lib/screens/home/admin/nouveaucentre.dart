...
class AjouterCentrePage extends StatefulWidget {
  ...
  final String? centreId; // optionnel : id du document Firestore si tu veux que la page mette à jour directement
...
class _AjouterCentrePageState extends State<AjouterCentrePage> {
  ...
  // Méthode wrapper pour appeler ton service d'enregistrement (garde l'abstraction si ton service a une signature différente)
  Future<void> _firebase_service_registerOrCreate(Map<String, dynamic> centreMap, String password) async {
    // Si ton FirebaseService.registerCentre attend des paramètres nommés (comme dans ton code),
    // adapte ici l'appel. J'écris un wrapper pour garder le point central.
    await _firebaseService.registerCentre(
